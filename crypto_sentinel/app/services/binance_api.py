...
async def get_crypto_price(symbol:str="BTC",http_session:aiohttp.ClientSession=None)->float|None:
    pair=f"{symbol.upper()}USDT"
    redis_key = f"price:{pair}"
    cached_price=await redis_client.get(redis_key)
    if cached_price:
        return float(cached_price)
    url = f"https://api4.binance.com/api/v3/ticker/price"
    try:
        async with http_session.get(url, params={"symbol": pair},timeout=5) as response:
